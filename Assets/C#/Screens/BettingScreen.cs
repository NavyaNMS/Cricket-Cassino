...
public class BettingScreen : Screen
{
    ...
    public override void Initialize(ScreenController screenController)
    {
        ...
        DefaulValue();
        test.onClick.AddListener(() => animController.PlayAnimation(UnityEngine.Random.Range(0, 99)));
        AddListener();
    }
    void DefaulValue()
    {
    ...
    void AddListener()
    {
        ...
        for (int i = 0; i < drawTimeToggles.Length; i++)
        {
            ...
            drawTimeToggles[i].onValueChanged.AddListener((isOn) =>
            {
                if (!isOn) return;
    ...
    void OnDrawResult(WinNo obj)
    {
        ...
        animController.PlayAnimation(gameId);
        LoadData();
    ...
    private void PrintTicket(BetValidation bet)
    {
        ...
        Printer print = new Printer(currentTime, date,
            totalSpots.ToString(),currentTicket
            ,
            betsContainer, poolPrize,serviceCharge);
        recentPrint = print;
