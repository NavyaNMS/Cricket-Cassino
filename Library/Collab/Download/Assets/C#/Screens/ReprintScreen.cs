...
public class ReprintScreen : Screen
{
    ...
    void PopulateContent(ThisRoundBets obj)
    {
        ...
        for (int i = 0; i < obj.data.Count; i++)
        {
            ...
            Printer p = new Printer(obj.data[i].draw_time, obj.data[i].playing_date, totalSpots.ToString(), obj.data[i].draw_time,
                obj.data[i].barcode_no, 
                bets,poolPriz,serviseChrge,(GameIds)int.Parse(obj.data[i].game_id));
